import { ArrowLeft, BookOpen, ChevronRight, CircleHelp, ClipboardCheck, Code2, Crown, Gamepad2, ImageUp, KeyRound, LockKeyhole, LogOut, Medal, MessageCircleMore, Play, Settings, Sparkles, Star, Trophy, UserRound, Zap } from 'lucide-react'
import { useEffect, useMemo, useState } from 'react'
import { useNavigate } from 'react-router-dom'
import PythonLab from './PythonLab'
import { awardQuestionPoints, baseLeaderboard, demoClass, demoLessons, getStudentProgress, getStudentSession, logoutStudent } from '../../lib/studentPortal'
...
export default function StudentPortal() {
  ...
  const [points, setPoints] = useState(() => session ? getStudentProgress(session.studentId).points : 0)
  const [toast, setToast] = useState('')
  const [passwordOpen, setPasswordOpen] = useState(false)

  useEffect(() => { if (!session) navigate('/student/login', { replace: true }) }, [navigate, session])
  useEffect(() => {
    const receive = (event: MessageEvent<ScoreMessage>) => {
      if (event.origin !== window.location.origin || event.data?.type !== 'growth-courseware:score' || !session) return
  ...
  return (
    <div className="quest-app">
      ...
      {view === 'lobby' && <Lobby studentName={session.studentName} points={points} onOpen={setView} />}
      {view === 'classroom' && <Classroom studentName={session.studentName} points={points} onBack={() => setView('lobby')} />}
...
function Lobby({ studentName, points, onOpen }: { studentName: string; points: number; onOpen: (view: View) => void }) {
  return <main className="quest-lobby">
    ...
    <section className="lobby-status"><div><Trophy /><span><small>当前积分</small><strong>{points}</strong></span></div><div><BookOpen /><span><small>课程进度</small><strong>{demoClass.progress} / {demoClass.totalLessons}</strong></span></div><div><Medal /><span><small>班级排名</small><strong>第 5 名</strong></span></div><div className="status-motto"><span>今日提示</span><p>把大问题拆成小问题，代码就会变简单。</p></div></section>
  </main>
}
...
function Classroom({ studentName, points, onBack }: { studentName: string; points: number; onBack: () => void }) {
  ...
  return <main className="classroom-page">
    <div className="classroom-heading"><button type="button" onClick={onBack}><ArrowLeft /> 返回冒险大厅</button><div><span>LEVEL SELECT</span><h1>{demoClass.name}</h1><p>{demoClass.teacher} · 已完成 {demoClass.progress}/{demoClass.totalLessons} 节课</p></div><div className="class-progress-ring"><strong>{Math.round(demoClass.progress / demoClass.totalLessons * 100)}%</strong><small>课程进度</small></div></div>
    <div className="classroom-layout">
      <aside className="lesson-map"><div className="map-title"><span><Gamepad2 /></span><div><small>COURSE MAP</small><strong>课程关卡</strong></div></div>{demoLessons.map((lesson) => <button type="button" key={lesson.id} disabled={lesson.status === 'locked'} className={`${lesson.id === lessonId ? 'active' : ''} ${lesson.status}`} onClick={() => setLessonId(lesson.id)}><span>{lesson.status === 'locked' ? <LockKeyhole /> : lesson.sequence}</span><div><strong>{lesson.title}</strong><small>{lesson.subtitle}</small></div><ChevronRight /></button>)}</aside>
      <section className="lesson-stage"><div className="stage-top"><div><span>LEVEL {String(lesson.sequence).padStart(2, '0')}</span><h2>{lesson.title}</h2><p>{lesson.subtitle}。认真观看课件，答题积分会自动记录。</p></div><div className="stage-reward"><Star /><span><small>本课奖励</small><strong>闯关积分</strong></span></div></div><div className="courseware-frame"><div className="frame-label"><i /><span>互动课件</span><small>方向键或空格翻页 · 答题结果自动记录</small></div><iframe key={lesson.id} title={`${lesson.title}互动课件`} src={coursewareSrc} allow="fullscreen" /></div><PythonLab /></section>
