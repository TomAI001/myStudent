...
export default function StudentPortal() {
  ...
  return (
    <div className="quest-app">
      ...
      {view === 'classroom' && <Classroom studentName={session.studentName} points={points} studentProgress={studentProgress} completionSignal={completionSignal} features={features} onLessonChange={setActiveLessonId} onBack={() => setView('lobby')} />}
...
function Classroom({ studentName, points, studentProgress, features, onLessonChange, onBack }: { studentName: string; points: number; studentProgress: StudentProgress; completionSignal: CompletionSignal; features:StudentFeatureState|null; onLessonChange:(lessonId:string)=>void; onBack: () => void }) {
  ...
  const coursewareSrc = coursewareBase ? `${coursewareBase}${coursewareBase.includes('?')?'&':'?'}studentId=${encodeURIComponent(features?.student.id||studentName)}&studentName=${encodeURIComponent(studentName)}&points=${points}` : ''
