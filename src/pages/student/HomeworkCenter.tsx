...
function HomeworkSubmit({ assignment, studentId, studentName, previous, onBack }: { assignment: HomeworkAssignment; studentId: string; studentName: string; previous: ReturnType<typeof getLearningState>['homeworkSubmissions']; onBack: () => void }) {
  ...
  const [output, setOutput] = useState('运行代码后，这里会显示结果。'); const [running, setRunning] = useState(false); const [done, setDone] = useState(false); const [uploading, setUploading] = useState(false)
  ...
  const submit = async () => {
    ...
    try {
      ...
      for (const file of photos) uploadedPhotos.push((await uploadPublicFile(file, `homework/${studentId}/${assignment.id}`)).url)
      const uploadedVideo = video ? (await uploadPublicFile(video, `homework/${studentId}/${assignment.id}`)).url : null
      const state = getLearningState(); state.homeworkSubmissions.push({ id:createId('homework-submit'),homeworkId:assignment.id,studentId,studentName,code,note,photos:uploadedPhotos,video:uploadedVideo,submittedAt:new Date().toISOString(),attempt:previous.length+1,score:null,feedback:'' }); saveLearningState(state); setDone(true)
    } catch (reason) { window.alert(reason instanceof Error ? reason.message : '作业附件上传失败，请稍后再试。') }
